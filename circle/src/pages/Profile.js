import React from "react";

function Profile({user}) {
    
  const username = async () => {

    const response = await fetch(`/items/${user._username}`)
    if (response.status === 200) {
      
      return response;
    } else {
      alert(`Error fectching ${user._username} `)
    }
};
    return (
      <>
      <div class="flex-container">
        <div class="left-col">
          <div class="map-image">
            <img src="https://staticmapmaker.com/img/google-placeholder.png" alt=""/>
          </div>
        </div>
        <div class="middle-col">
          <div class="circle-title">
            <div class="circle-title-1">
              <h2>${username}</h2>
              <p class="circl-intro">We like San Diego!</p>
            </div>
            <div class="circle-title-2"><button>Follow</button></div>
          </div>
          <div class="posts">
            <div class="post-title">
              <img
                class="profile-image"
                alt = ""
                src="data:image/jpeg;base64,"
                />
              <p class="user-name">Jay</p>
            </div>
            <div class="post-content">
              <p class="post-text">
                Lorem ipsum dolor sit amet, pri ridens moderatius scripserit id,
                reque definitionem ex usu. Ne has affert denique
                conclusionemque. At vix tale epicurei probatus. Ne duo convenire
                consulatu interesset. Ne quo eruditi reprehendunt, te accusata
                praesent vix. Nulla appareat vituperatoribus ius ad.
              </p>
              <img
                class="post-image"
                alt=""
                src="https://zoo.sandiegozoo.org/sites/default/files/body_side_image/Plan%20Your%20Visit-Direcions%20%26%20Parking.jpg"
              />
              <div class="reactions">
                <button>Like</button>
                <button>Share</button>
                <button>Comment</button>
                <textarea placeholder="comment"></textarea>
                <div class="post-comment-button">
                  <button>Post Comment</button>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div class="right-col">
          <div class="holder">
            <h5>10 circlers</h5>
            <div class="circler">
              Tongxin Sun
              <div class="align-right">
                <button class="profile">Pr</button>
                <button class="dm">Dm</button>
              </div>
            </div>
            <div class="circler">
              Tongxin Sun
              <div class="align-right">
                <button class="profile">Pr</button>
                <button class="dm">Dm</button>
              </div>
            </div>
          </div>
          <div class="holder">
            <h5>Similar circles</h5>
            <div id="circle-holder">
              <div class="circle" id="circle">San</div>
          
        <div class="circle" id="circle">Friends in LA</div>
              <div class="circle" id="circle">Trip to USA 2021</div>
              <div class="circle" id="circle">Trip to Japan 202</div>
              <div class="circle" id="circle">Trip to USA 2021</div>
            </div>
          </div>
        </div>
      </div>
      </>)};

export default Profile;